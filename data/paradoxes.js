...
export function getParadoxesBySource(source = 'all') {
    if (source === 'all') return paradoxes;
    return paradoxes.filter(p => p.source === source);
}

export function getParadoxesByCategory(categoryId, source = 'all') {
    const filtered = source === 'all' ? paradoxes : paradoxes.filter(p => p.source === source);
    return filtered.filter(p => p.category === categoryId);
}

export function getParadoxesByPoints(descending = true, source = 'all') {
    const filtered = source === 'all' ? paradoxes : paradoxes.filter(p => p.source === source);
